...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login'])) {
    ...
    // Check if the username and password match (plain text comparison)
    $sql = "SELECT * FROM admin_user WHERE username = ? AND password = ?"; // Plain text comparison
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $login_username, $login_password); // Bind plain password
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['register'])) {
    ...
    $register_password = mysqli_real_escape_string($conn, $_POST['register_password']); // Plain text password
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt->bind_param("sss", $register_username, $register_email, $register_password); // Insert plain password
...
<script>
    ...
    function animateBall(ball) {
        ...
        ball.animate(
            ...
            {
                duration: Math.random() * 4000 + 2000, // Random duration between 2 and 6 seconds
                iterations: Infinity, // Infinite loop
                direction: 'alternate', // Back and forth movement
            }
        );
    }

    // Generate 10 random balls on page load
